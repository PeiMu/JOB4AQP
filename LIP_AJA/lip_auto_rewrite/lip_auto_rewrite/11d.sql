...
SELECT sum(pg_lip_bloom_add(0, cn.id)) FROM company_name AS cn WHERE cn.country_code !='[pl]';
...
/*+
NestLoop(k mk ml t lt mc ct cn)
NestLoop(k mk ml t lt mc ct)
NestLoop(k mk ml t lt mc)
NestLoop(k mk ml t lt)
NestLoop(k mk ml t)
NestLoop(k mk ml)
NestLoop(k mk)
SeqScan(k)
