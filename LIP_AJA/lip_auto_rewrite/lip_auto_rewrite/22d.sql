...
SELECT sum(pg_lip_bloom_add(5, mi.movie_id)) FROM movie_info AS mi WHERE mi.info IN ('Sweden', 'Norway', 'Germany', 'Denmark', 'Swedish', 'Danish', 'Norwegian', 'German', 'USA', 'American');
SELECT sum(pg_lip_bloom_add(6, mi_idx.movie_id)) FROM movie_info_idx AS mi_idx WHERE mi_idx.info < '8.5';
SELECT sum(pg_lip_bloom_add(7, t.id)) FROM title AS t WHERE t.production_year > 2005;

/*+
NestLoop(it2 k mk t kt mi_idx mi it1 mc cn ct)
NestLoop(it2 k mk t kt mi_idx mi it1 mc cn)
NestLoop(it2 k mk t kt mi_idx mi it1 mc)
