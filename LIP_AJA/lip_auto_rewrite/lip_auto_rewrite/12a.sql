...
SELECT sum(pg_lip_bloom_add(4, mi.movie_id)) FROM movie_info AS mi WHERE mi.info IN ('Drama', 'Horror');
SELECT sum(pg_lip_bloom_add(5, mi_idx.movie_id)) FROM movie_info_idx AS mi_idx WHERE mi_idx.info > '8.0';
SELECT sum(pg_lip_bloom_add(6, t.id)) FROM title AS t WHERE t.production_year between 2005 and 2008;

/*+
NestLoop(mi_idx it2 mc ct t cn mi it1)
NestLoop(mi_idx it2 mc ct t cn mi)
NestLoop(mi_idx it2 mc ct t cn)
NestLoop(mi_idx it2 mc ct t)
HashJoin(mi_idx it2 mc ct)
NestLoop(mi_idx it2 mc)
HashJoin(mi_idx it2)
SeqScan(mi_idx)
SeqScan(it2)
IndexScan(mc)
