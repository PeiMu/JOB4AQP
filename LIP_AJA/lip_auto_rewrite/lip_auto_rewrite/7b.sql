...
SELECT sum(pg_lip_bloom_add(3, n.id)) FROM name AS n WHERE n.name_pcode_cf LIKE 'D%' AND n.gender='m';
SELECT sum(pg_lip_bloom_add(4, pi.id)) FROM person_info AS pi WHERE pi.note ='Volker Boehm';
SELECT sum(pg_lip_bloom_add(5, t.id)) FROM title AS t WHERE t.production_year between 1980 and 1984;
