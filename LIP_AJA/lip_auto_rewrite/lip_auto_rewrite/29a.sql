...
SELECT sum(pg_lip_bloom_add(6, it3.id)) FROM info_type AS it3 WHERE it3.info = 'trivia';
SELECT sum(pg_lip_bloom_add(7, k.id)) FROM keyword AS k WHERE k.keyword = 'computer-animation';
SELECT sum(pg_lip_bloom_add(8, mi.movie_id)) FROM movie_info AS mi WHERE mi.info IS NOT NULL AND (mi.info LIKE 'Japan:%200%' OR mi.info LIKE 'USA:%200%');
...
SELECT sum(pg_lip_bloom_add(11, t.id)) FROM title AS t WHERE t.title = 'Shrek 2' AND t.production_year between 2000 and 2010;
