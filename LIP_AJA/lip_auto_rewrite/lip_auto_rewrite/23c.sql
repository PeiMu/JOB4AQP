...
SELECT sum(pg_lip_bloom_add(3, kt.id)) FROM kind_type AS kt WHERE kt.kind IN ('movie', 'tv movie', 'video movie', 'video game');
...
SELECT sum(pg_lip_bloom_add(5, t.id)) FROM title AS t WHERE t.production_year > 1990;
