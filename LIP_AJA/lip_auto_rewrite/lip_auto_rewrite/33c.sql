...
SELECT sum(pg_lip_bloom_add(3, kt1.id)) FROM kind_type AS kt1 WHERE kt1.kind IN ('tv series', 'episode');
SELECT sum(pg_lip_bloom_add(4, kt2.id)) FROM kind_type AS kt2 WHERE kt2.kind IN ('tv series', 'episode');
SELECT sum(pg_lip_bloom_add(5, lt.id)) FROM link_type AS lt WHERE lt.link IN ('sequel', 'follows', 'followed by');
SELECT sum(pg_lip_bloom_add(6, mi_idx2.id)) FROM movie_info_idx AS mi_idx2 WHERE mi_idx2.info < '3.5';
SELECT sum(pg_lip_bloom_add(7, t2.id)) FROM title AS t2 WHERE t2.production_year between 2000 and 2010;

/*+
NestLoop(mi_idx1 ml lt it1 t1 kt1 mi_idx2 it2 t2 mc1 cn1 kt2 mc2 cn2)
NestLoop(mi_idx1 ml lt it1 t1 kt1 mi_idx2 it2 t2 mc1 cn1 kt2 mc2)
NestLoop(mi_idx1 ml lt it1 t1 kt1 mi_idx2 it2 t2 mc1 cn1 kt2)
NestLoop(mi_idx1 ml lt it1 t1 kt1 mi_idx2 it2 t2 mc1 cn1)
NestLoop(mi_idx1 ml lt it1 t1 kt1 mi_idx2 it2 t2 mc1)
NestLoop(mi_idx1 ml lt it1 t1 kt1 mi_idx2 it2 t2)
