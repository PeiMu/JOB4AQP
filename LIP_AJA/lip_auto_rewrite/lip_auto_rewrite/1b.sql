...
SELECT sum(pg_lip_bloom_add(1, it.id)) FROM info_type AS it WHERE it.info = 'bottom 10 rank';
SELECT sum(pg_lip_bloom_add(2, mc.movie_id)) FROM movie_companies AS mc WHERE mc.note NOT LIKE '%(as Metro-Goldwyn-Mayer Pictures)%';
SELECT sum(pg_lip_bloom_add(3, t.id)) FROM title AS t WHERE t.production_year between 2005 and 2010;
