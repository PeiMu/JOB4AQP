...
SELECT sum(pg_lip_bloom_add(1, mi.movie_id)) FROM movie_info AS mi WHERE mi.info IN ('Sweden', 'Norway', 'Germany', 'Denmark', 'Swedish', 'Denish', 'Norwegian', 'German');
SELECT sum(pg_lip_bloom_add(2, t.id)) FROM title AS t WHERE t.production_year > 2005;
