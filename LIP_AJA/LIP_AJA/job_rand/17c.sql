...
SELECT sum(pg_lip_bloom_add(1, id)) FROM name AS n WHERE n.name LIKE 'X%'; -- filter on mc.company_id

/*+
NestLoop(k mk ci n mc cn t)
NestLoop(k mk ci n mc cn)
NestLoop(k mk ci n mc)
NestLoop(k mk ci n)
NestLoop(k mk ci)
NestLoop(k mk)
