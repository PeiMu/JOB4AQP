...
-- DROP MATERIALIZED VIEW IF EXISTS  cast_info_bf;
-- CREATE MATERIALIZED VIEW cast_info_bf AS (
--    SELECT *, pg_lip_bloom_add(0, movie_id)  FROM cast_info AS ci WHERE ci.note IN ('(writer)',
--                   '(head writer)',
--                   '(written by)',
--                   '(story)',
--                   '(story editor)') -- AND pg_lip_bloom_probe(5, ci.person_id) 
-- );
-- CREATE INDEX cast_info_bf_movie_id ON cast_info_bf( movie_id );

-- DROP MATERIALIZED VIEW IF EXISTS movie_info_bf;
-- CREATE MATERIALIZED VIEW movie_info_bf AS (
--    SELECT *,pg_lip_bloom_add(4, movie_id)  FROM movie_info AS mi WHERE mi.info IN ('Horror',
--                   'Action',
--                   'Sci-Fi',
--                   'Thriller',
--                   'Crime',
--                   'War')  AND  pg_lip_bloom_probe(1, info_type_id) AND  pg_lip_bloom_probe(0, movie_id) 
-- );
-- CREATE INDEX movie_info_bf_movie_id ON movie_info_bf( movie_id );
-- SELECT sum(pg_lip_bloom_bit_and(6, 6, 4));
